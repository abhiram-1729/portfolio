      ...
      <div className="space-y-6 animate-in slide-in-from-right duration-500">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10 pb-8 border-b border-gray-50">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><FileText size={12} className="text-emerald-400" /> Admin User</span>
              <span className="text-sm font-black text-gray-800">{audit.user?.name}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Truck size={12} className="text-emerald-400" /> Vehicle Type</span>
              <span className="text-sm font-black text-gray-800">{audit.vehicle?.vehicleName || 'Standard Load'}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><RefreshCw size={12} className="text-emerald-400" /> Timestamp</span>
              <span className="text-sm font-black text-gray-800">
                {new Date(audit.createdAt).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit', hour12: true })}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Package size={12} className="text-emerald-400" /> Items Changed</span>
              <span className="text-sm font-black text-gray-800">{audit.items.length} Products</span>
            </div>
            <div className="flex flex-col col-span-full mt-4 bg-emerald-50/30 p-5 rounded-2xl border border-emerald-100/50">
              <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Gift size={12} /> Auditor's Remark</span>
              <span className="text-sm font-black text-emerald-950 italic leading-relaxed">"{audit.remark || 'No session remarks provided for this adjustment.'}"</span>
            </div>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black text-emerald-600 uppercase tracking-widest text-center">Audited Qty</th>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-emerald-50/10 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-black text-gray-800 group-hover:text-emerald-600 transition-colors uppercase tracking-tight leading-none mb-1">{item.product?.name}</span>
                      ...
                      <td className="px-6 py-4 text-sm font-black text-emerald-600 text-center bg-emerald-50/5 font-mono">{item.newQuantity}</td>
